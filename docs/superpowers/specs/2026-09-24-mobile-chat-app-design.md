...
| Approving an action | Asks for the PIN or biometrics at the moment of the tap only for an irreversible action or "Permitir sempre nesta aba"; a write action approves with the unlocked session, like denying (TER-92). |
...
- **Approving a pending action**: when the PIN is required (the action's class is not `write`, or the decision is `approve_tab`), the body carries a fresh challenge (`purpose: 'decision'`, bound to the action id) and `pin_proof = HMAC(pin_secret, challenge ‖ action_id ‖ 'approve')`; otherwise both are omitted. `POST chat/actions/:id/decision` with `decision: 'approve'` answers `401 PIN_REQUIRED` when the proof is required and missing. `deny` needs nothing beyond the normal proof of possession.
...
| Stolen phone, unlocked, or rooted | PIN on open and after 5 min; irreversible approvals and tab grants ask for the PIN; write approvals rely on the unlocked session (PIN on open and after 5 min in the background); tag-less wrapping makes offline PIN testing impossible; 3 failures lock, 6 revoke, counted on the server. |

...
- **Approving a pending action** asks for the PIN only when the action's class is not `write` or the decision is `approve_tab`; otherwise `pin_proof` is optional. `POST chat/actions/:id/decision` with `decision: 'approve'` needs a fresh challenge (`purpose: 'decision'`, bound to the action id) and `pin_proof = HMAC(pin_secret, challenge ‖ action_id ‖ 'approve')` when required; missing when required answers `401 PIN_REQUIRED`. `deny` needs nothing beyond the normal proof of possession.

...
| Interaction with TER-92 | TER-92 (parallel branch) makes `write` approvals on the phone PIN-free. Both landed (2026-09-26): the batch route follows the same rule as the single one — a `write` approval goes without a proof, any other class needs its own, and one missing proof refuses the whole batch with `PIN_REQUIRED` before any challenge is spent; a proof sent anyway is checked. The app sends a write-only batch without the PIN (falling back to the sheet on `PIN_REQUIRED`/`VALIDATION`); a batch with an irreversible approval asks the PIN once and proves every approval. "Permitir sempre" is never part of a batch. |
